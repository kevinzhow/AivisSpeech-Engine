...
def _add_style_bert_vits2_metadata(gguf: Any, writer: Any, config: dict[str, Any]) -> None:
    ...
    writer.add_uint32(f"{arch}.sample_rate", _int_at(data_config, "sampling_rate", 44100))
    writer.add_uint32(f"{arch}.inter_channels", _int_at(model_config, "inter_channels", 192))
    writer.add_uint32(f"{arch}.hidden_channels", _int_at(model_config, "hidden_channels", 192))
    writer.add_uint32(f"{arch}.filter_channels", _int_at(model_config, "filter_channels", 768))
    writer.add_uint32(f"{arch}.n_heads", _int_at(model_config, "n_heads", 2))
    writer.add_uint32(f"{arch}.text_encoder.n_layers", _int_at(model_config, "n_layers", 6))
    writer.add_uint32(f"{arch}.text_encoder.kernel", _int_at(model_config, "kernel_size", 3))
    writer.add_uint32(f"{arch}.text_encoder.window_size", 4)
    writer.add_uint32(f"{arch}.text_encoder.cond_layer_idx", _int_at(model_config, "cond_layer_idx", 2))
    writer.add_uint32(f"{arch}.gin_channels", _int_at(model_config, "gin_channels", 512))
    writer.add_uint32(f"{arch}.jp_extra", 1 if str(config.get("version", "")).endswith("JP-Extra") else 0)
    writer.add_uint32(
    ...
    writer.add_uint32(
        ...
        _int_at(model_config, "kernel_size_dp", _int_at(model_config, "kernel_size", 3)) // 2,
    )
    ...
    writer.add_uint32(f"{arch}.flow.n_layers", _int_at(model_config, "n_layers_flow", 6))
    writer.add_uint32(f"{arch}.flow.kernel", _int_at(model_config, "kernel_size_flow", 5))
    ...
    for index in range(num_upsamples):
        ...
        writer.add_uint32(f"{arch}.decoder.ups.{index}.padding", max((kernel - stride) // 2, 0))
    ...
    for block_index in range(num_upsamples * num_kernels):
        ...
        dilations = _matrix_row(resblock_dilations, block_index % num_kernels, (1, 3, 5))
